...
int ReadOpenLr2Config(game* g, const char* filepath) {
	...
	ReadXml_PositiveIntAsBool("config", "play", "newrandomrestart", true, &g->config.play.newRandomRestart, hXml);
	...
	ReadXml_Int("config", "play", "lift", 0, &g->config.play.liftv[PLAYER_1], hXml);
	g->config.play.liftv[PLAYER_2] = g->config.play.liftv[PLAYER_1];
